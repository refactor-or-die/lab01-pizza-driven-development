...
class place_order:
    """zarzadza calym zamowieniem po sronie systemu"""
    def place_order( pizza_type, address, delivery_time, card_number, user_id):
        # Utwórz instancje wymaganych klas
        inventory_manager = InventoryManager()
        price_calculator = PriceCalculator()
        payment_processor = PaymentProcessor()
        delivery_scheduler = DeliveryScheduler()
        loyalty_points_calculator = LoyaltyPointsCalculator()
        notification_service = NotificationService()

        # Krok 1: Sprawdź dostępność pizzy
        if not inventory_manager.check_availability(pizza_type):
            return {
                "success": False,
        ...
        # Krok 2: Oblicz cenę
        price = price_calculator.get_price(pizza_type)
        if price == 0.0:
            return {
                "success": False,
        ...
        if not payment_processor.process_payment(card_number, price):
            return {
                "success": False,
        ...
        if not inventory_manager.reserve_pizza(pizza_type):
            return {
                "success": False,
        ...
        delivery_id = delivery_scheduler.schedule_delivery(address, delivery_time)
        
        # Krok 6: Dodaj punkty lojalnościowe
        points_earned = loyalty_points_calculator.add_points(user_id, price)
...
def place_pizza_order(pizza_type, address, delivery_time, card_number, user_id):
    ...
    return place_order.place_order(pizza_type, address, delivery_time, card_number, user_id)
